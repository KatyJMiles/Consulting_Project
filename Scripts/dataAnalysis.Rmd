...
names(p) = rep(names(sum), each = 2)
plist_adjusted = p.adjust(p, "fdr")

pval_df_manova = merge(stack(p), 
                stack(plist_adjusted), 
                by="ind") %>%
  mutate("Sig" = ifelse(values.y <= 0.05, "Yes", "No"))
colnames(pval_df_manova) = c("response", "pval_raw", "pval_adj", "sig")

write.csv(pval_df_manova, "../Output/pval_df_manova.csv", row.names = FALSE)
